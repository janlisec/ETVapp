...
``` r
library(ETVapp)
td <- ETVapp::ETVapp_testdata[['ExtCal']]
...
time_col <- "Time"
int_col <- "157"
cali_pro <- process_data(data = cali_imp, c1 = int_col, fl = 9, wf = "ExtCal")
gt::gt(head(cali_pro[[1]]))
```
...
![](workflow_ExtCal2_files/figure-html/cali_peak_detection_plot-1.png)
...
![](workflow_ExtCal2_files/figure-html/calibration_model-1.png)
...
``` r
smpl_pro <- process_data(data = td[['Sample']][[1]], c1 = int_col, fl = 9, wf = "ExtCal")
...
#> BLmethod 'none' selected. BL correction step omitted.
gt::gt(smpl_peak)
```
...
![](workflow_ExtCal2_files/figure-html/sample_peak_detection_plot-1.png)
...
``` r
blnk_pro <- process_data(data = td[['Blanks']], c1 = int_col, fl = 9, wf = "ExtCal")
...
![](workflow_ExtCal2_files/figure-html/load_blanks-1.png)
...
``` r
gt::gt(tab_LOX(x = blnk_peaks[,4], cali_slope = cm[1,1], wf = "ExtCal", mass_fraction2 = 1, sample_mass = 1))
```
