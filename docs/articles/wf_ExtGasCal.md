...
anlt <- "13C"
time_col <- "Time"
cali_pro <- process_data(data = cali_imp, c1 = anlt, fl = 9, wf = wf)
...
![](wf_ExtGasCal_files/figure-html/cali_peak_detection_plot-1.png)

Peak areas are plotted against the gas flows. Calibration model
parameters obtained through linear regression are stored for later use.

``` r
cm <- calc_cali_mod(df = cali_peaks[,c(5,4)], wf = wf)
par(mfrow=c(1,1))
plot(cali_peaks[,c(5,4)])
abline(a = cm[1,3], b = cm[1,1])
```

![](wf_ExtGasCal_files/figure-html/calibration_model-1.png)
...
Repeat the processing procedure for the sample data.

``` r
smpl_pro <- process_data(data = td[['Samples']], c1 = anlt, fl = 9, wf = wf)
str(smpl_pro[[1]])
...
![](wf_ExtGasCal_files/figure-html/sample_peak_detection_plot-1.png)
...
``` r
blnk_pro <- process_data(data = td[['Blanks']], c1 = anlt, fl = 9, wf = wf)
...
![](wf_ExtGasCal_files/figure-html/load_blanks-1.png)
