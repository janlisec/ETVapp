...
![](workflow_IDMS2_files/figure-html/mass_bias_peak_detection_plot-1.png)
...
| Spike isotope | Sample isotope | R_m      |
|---------------|----------------|----------|
| 113Cd         | 111Cd          | 1.349456 |
| 113Cd         | 111Cd          | 1.660122 |
| 113Cd         | 111Cd          | 1.653288 |

Plot the sample time scans with the peak parameter for each isotope.

``` r
samp_pro <- process_data(data =samp_imp, wf = "IDMS", c1 = iso1, c2 = iso2, fl = NULL)
...
for (i in 1:3) {
  plot(samp_pro[[i]][,c("Time")], samp_pro[[i]][,c("113Cd")], type="l", 
       ylab = "Intensity [cps]", xlab = "Time [s]", main = paste("113Cd",  "Sample", samp_no[i]))
  #lines(mb_pro[[i]][,c("Time")], mb_pro[[i]][,c("111Cd")], col=3)
...
for (i in 1:3) {
  plot(samp_pro[[i]][,c("Time")], samp_pro[[i]][,c("111Cd")], type="l", 
       ylab = "Intensity [cps]", xlab = "Time [s]", main = paste("111Cd", "Sample", samp_no[i]))
  #lines(mb_pro[[i]][,c("Time")], mb_pro[[i]][,c("111Cd")], col=3)
...
![](workflow_IDMS2_files/figure-html/sample_peak_detection_plot-1.png)
...
|----|----|----|----|----|----|----|
| 113Cd | 111Cd | 1.349456 | 1.284645 | 0.3261680 | 1 | 0.3261680 |
| 113Cd | 111Cd | 1.660122 | 1.580391 | 0.1718800 | 1 | 0.1718800 |
| 113Cd | 111Cd | 1.653288 | 1.573885 | 0.1736885 | 1 | 0.1736885 |
...
``` r
gt::gt(tab_LOX(x = LOX_df[,"R_corr"], wf = "IDMS"))
...
|----|----|----|----|----|
| 0.506706 | 1.68902 | 1 | 0.506706 | 1.68902 |
