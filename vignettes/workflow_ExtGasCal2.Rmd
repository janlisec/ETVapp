...
ICP-MS and ETV/ICP-MS [paper citation]. The working principle is based on the 
...
included in [paper](https-Link). They can be accessed by `ETVapp::ETVapp_testdata`.
...
cali_peaks <- tab_cali(peak_data = cali_sig, wf = "ExtGasCal", std_info = seq(0,50,length.out=nrow(cali_sig)), fac = fac)
print(cali_peaks)
```
